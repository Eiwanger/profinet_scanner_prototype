	...
	while (bl_count < ntohs(profinet->dataLength)){
		...
		case OPT_DevicePropertiesOption:
			...
			case SUBOPT_DPO_DeviceVendor:;
				...
				recData->deviceType = (char*)malloc(sizeof(char)*(pn_block->blocklength - 2 + 1)); // -2 because of Blockinfo and +1 for termination '\0'
				if (!recData->deviceType)
				{
...
// is called for each packet which goes through the filter
// different from packet_handler_IP only through way of comparing data in the list with current data (here mac address)
void packet_handler_IP(u_char* param, const struct pcap_pkthdr *header, const u_char *pkt_data)
{
	...
	while (tmpList != NULL)
	{
		if (compareMacAddress(tmpList->device->deviceMACaddress, recData->deviceMACaddress))
		{
			if (tmpList->device->deviceType == NULL){
...
// parses packets which have the option 0x05 implicit read 
void packet_handler_ImplicitRead(threadData_t* threadData, const struct pcap_pkthdr *header, const u_char * pkt_data){
	...
	case REALIDENTIFICATIONDATA:
		...
		break;
	case IM0:;	// now data for each module
		getIMData(pn_readimplicit, currentDevice);
...
// function to capture and parse dcp packets
...
// captures and parses IP/UDP packets
int captureIPPackets(threadData_t* threadData){
	pcap_if_t *d;
	int i = 0;
	char errbuf[PCAP_ERRBUF_SIZE];
	u_int netmask;
	char packet_filter[] = "ether proto 0x0800 and ip proto 0x11"; // set a filter
...
// cut the hardwarerevision from a give versions string
...
char* cutSoftVersion(char* versions)
{
	...
	//count whole string starting with 'V'
	size = strlen(pos);
...
char* cutDataFromString(u_char *annotation, int* offset, bool lastItem)
{
	char *destination = (char*)malloc(sizeof(char) * 33);
	if (!destination)
	{
	...
	for (int j = 0; j < 32; j++, i++)
	{
		...
		if (annotation[i] == 0x20 && annotation[i + 1] == 0x20 && annotation[i + 2] == 0x20 && !lastItem){
			*offset = i;
			destination = realloc(destination, sizeof(char)*(j + 1));
			if (!destination){
				printf("Error reallocating memory for destination string\n");
				return NULL;
			}
			destination[j] = '\0';
			return destination; // end of important string; 
		}
		else if (lastItem && i == 63){
			destination = realloc(destination, sizeof(char)*(j + 1));
			if (!destination){
				printf("Error reallocating memory for destination string\n");
				return NULL;
...
void getRealidentificationData(pn_ReadImplicit* pn_readimplicit, linked_list_t* currentDev){
	...
	if (numberofslots == 0) // no slots to read
		return;
...
void getSubmodulPDRealData(pn_ReadImplicit* pn_readimplicit, linked_list_t* currentDev)
{
	...
	// get port information own port id
	if ((subslots->ownPortID = malloc(pdPD->lengthOwnPortID + 1)) == NULL)
	{
		printf("Error allocating memory for ownPortId");
		return;
	}
	int i;

	for (i = 0; i < pdPD->lengthOwnPortID; i++)
	{
		subslots->ownPortID[i] = pdPD->ownPortID[i];
	}
	subslots->ownPortID[i] = '\0';
	...
	if (pdPD->numberOfPeers == 0)
	{
		...
		// MAUType
		subslots->MAUType = pdpdwp->MAUType;
		subslots->portState = ntohs(pdpdwp->portState);
	}
	else // we have at least one peer
	{
		pdPortDataWithPeer* pdpdWithP = (pdPortDataWithPeer*)&pdPD->pos;

		// extract peer port id
		if ((subslots->peerPortID = malloc(pdpdWithP->lengthPeerPortID + 1)) == NULL)
		{
			printf("Error allocating memory for ownPortId");
			return;
		}
		int i;
		for (i = 0; i < pdpdWithP->lengthPeerPortID; i++)
		{
			subslots->peerPortID[i] = pdpdWithP->peerPortID[i];
		}
		subslots->peerPortID[i] = '\0';
		...
		peerChassisID_p += i + 4 + ((4 - (pdpdWithP->lengthPeerChassisID + 62) % 4) % 4);
		...
		// MAUType
		peerChassisID_p += 6 + 2; // add macaddress and padding length
		subslots->MAUType = *(peerChassisID_p) << 8 | *(peerChassisID_p + 1);

		peerChassisID_p += 2 + 2 + 4 + 4; // add mautype padding domainboundary and multicastboundary length

		subslots->portState = *(peerChassisID_p) << 8 | *(peerChassisID_p + 1);
...
void getIMData(pn_ReadImplicit* pn_readimplicit, linked_list_t* currentDev){

	// get slot

	IM0Data* im0d = (IM0Data*)(((u_char*)(&(pn_readimplicit->padding_end[19]))) + 1);

	u_short slotnumber = ntohs(pn_readimplicit->slotNumber);
	linkedList_slot* slot = currentDev->device->slotList;
	...
	// now we have the slot
	slot->moduledata.vendorID = im0d->vendorIDHigh << 8 | im0d->vendorIDLow;
	int i;
	for (i = 0; i < 20; i++){
		slot->moduledata.orderID[i] = im0d->orderID[i];
		if (i < 16)
			slot->moduledata.IMserialNumber[i] = im0d->IMserialNumber[i];
	}
	slot->moduledata.orderID[i] = '\0';
	slot->moduledata.IMserialNumber[i - 4] = '\0';

	char buf[32];
	sprintf_s(buf, 32, "%c  %d  %d  %d", im0d->IMRevisionPrefix, im0d->IMSWRevisionFuncitonalEnhancement, im0d->IMSWRevisionBugfix, im0d->IMSWRevisionInternalChange);


	char* tmpstore = removeDuplicate(buf, (int)strlen(buf));
	int len = strlen(tmpstore);
	if ((slot->moduledata.version = malloc(len + 1)) == NULL)
	{
		printf("Error allocating memory for moduldata version");
		return;
	}

	for (i = 0; i < len; i++)
	{
		slot->moduledata.version[i] = tmpstore[i];
	}
	slot->moduledata.version[i] = '\0';



	slot->moduledata.IMProfileID = ntohs(im0d->IMProfileID);
	slot->moduledata.IMProfileSpecificType = im0d->IMProfileSpecificType;

	slot->moduledata.hardwareRevison = malloc(6); // size 99999\0
	if (!slot->moduledata.hardwareRevison)
	{
		printf("Error allocating memory for moduldata hardwarerevision");
		return;

	}

	sprintf_s(slot->moduledata.hardwareRevison, 6, "%d", ntohs(im0d->hardwareRevision));

	// first slot 0x0000 is the device itself
	if (slot->slotNumber == 0x0000)
	{
	...
	if (!slot->moduledata.IMversion)
	{
		printf("Error allocating memory for moduldata hardwarerevision");
		return;

	}
	sprintf_s(slot->moduledata.IMversion, 8, "%d.%d", im0d->IMVersionMajor, im0d->IMVersionMinor);

	slot->moduledata.IMSupported = ntohs(im0d->IMSupported);
...
/*
// open an adapter and start capturing packets
int packetCapture(pcap_if_t* onedev){
	u_int netmask;
	if (!onedev)
		return -1;
	...
	char errbuf[PCAP_ERRBUF_SIZE];
	// Open the adapter
	if ((adhandle = pcap_open_live(onedev->name,	// name of the device
		65536,		// portion of the packet to capture.
		1, //PCAP_OPENFLAG_PROMISCUOUS,			// promiscuous mode
		1000,		// read timeout
		errbuf		// error buffer
	...
	pcap_loop(adhandle, 0, packet_handler_IP, NULL);
