...
//	u_char destIp4;	// fourth byte destination ip
	...
	u_char srcPort1; // source port
	u_char srcPort2;

	u_char destPort1; // destination port
	u_char destPort2;

	u_short udp_length; // length of
...
typedef struct pdPortDataAfterPeerMAC{
	u_short padding1;
	u_short MAUType;
	u_short padding2;
	u_char  domainBoundary[4];
	u_char  multicastBoundary[4];
	u_short portState;
	u_short padding3;
	u_char mediaType[4];

}pdPortDataAfterPeerMAC;
