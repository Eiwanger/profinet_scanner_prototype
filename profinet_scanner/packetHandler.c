...
int sendPacket_RPC(threadData_t* threadData, int deviceNumber)
{
	pcap_t *fp;
...
// test possible reading of I&M data
/* PNIO-CM changes for different calls
	...
	rest looks identical
	=> parameter will be set at the callling function
	*/

int sendpacket_IM(threadData_t* threadData, int deviceNumber, u_short parameterIndex, int slotCount)
{
	pcap_t *fp;
	char errbuf[PCAP_ERRBUF_SIZE];
	...
	*/
	...
	// variable part
	packet_IM[60] = 0x00;
	packet_IM[61] = 0x01; // ? let's see if this works
	...
	if (parameterIndex == PDREALDATASUBMODUL || parameterIndex == PDREALDATASUBMODULEXT){
		// Slotnumber
		packet_IM[170] = 0x00;
		packet_IM[171] = 0x00;
